...
public class BookingController {
    ...
    public ResponseEntity<Page<BookingGridResponse>> findAll(@PathVariable UUID userId, @Valid @ModelAttribute BookingSearchRequest bookingSearchRequest) {
        ...
        log.info("Bookings fetched successfully by the user {}", userId);
        return ResponseEntity
    ...
    /* FIXME
    1. It could be improved including a cancelationToken (in the moment of booking creation) with expiration, e.g. 24 hs before booking (or able to be set by admin).
    2. The first step should be send an email, and then the capable user can invoke the cancelation with the token on the request.
     */
